...
function generateCarouselHTML(vendors: VendorData[]) {
  ...
  const vendorCards = vendors.map((vendor) => `<div class="vendor-card"><img src="${vendor.image_url}" alt="${vendor.name}" class="vendor-image"><div class="vendor-content"><div class="vendor-header"><h3 class="vendor-name">${vendor.name}</h3><div class="vendor-rating"><span class="star">★</span><span class="rating-value">${vendor.rating}</span><span class="review-count">(${vendor.review_count})</span></div></div><p class="vendor-description">${vendor.description}</p><div class="vendor-badges"><span class="badge">${vendor.price_range}</span>${vendor.is_licensed ? '<span class="badge badge-success">Licensed</span>' : ''}${vendor.is_insured ? '<span class="badge badge-success">Insured</span>' : ''}</div><div class="vendor-location">📍 ${vendor.city}, ${vendor.state}</div></div><button class="book-button" onclick="alert('Booking ${vendor.name}')">Book Now</button></div>`).join('')
...
// Create transport with JSON response mode
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create a complete response capture
